...
output_dir="scan_lfi_${timestamp}"
mkdir -p "$output_dir"
...
expect "Enter the path to the payloads file" { send "payloads/lfi.txt\r" }
expect "Enter the success criteria patterns" { send "\r" }
...
# === Fallback Xray ===
echo
echo "[→] Menjalankan fallback scan dengan Nuclei (LFI)..."
input_folder="$(dirname "$INPUTFILE")"
original_file="$input_folder/lfi_output.txt"
...
if [[ ! -f "$original_file" ]]; then
  ...
  "$script_dir/fallback_nuclei_lfi.sh" "$outputfile" "$original_file" "$session_id"

  nuclei_result_file="$HOME/Documents/projek-or/lfi/fallback_${session_id}/nuclei_lfi.txt"
...
"$script_dir/validate_lfi.sh" "$final_input" "$session_id"
