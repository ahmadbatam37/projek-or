...
echo "$(date +%s)" > "/tmp/.or_timer_${session_id}.tmp"

timestamp="$(date +"%Y%m%d-%H%M%S")_$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 4)"
basedir="$HOME/Documents/projek-or/open_redirect"
...
if [[ -n "$crawling_dir" && -d "$crawling_dir" ]]; then
    ...
    if [[ -f "$paramspider_output" ]]; then
        ...
        echo "⚠️ Paramspider tidak menghasilkan file: $paramspider_output"
...
# Step 6: Filter kemungkinan redirect
echo "[•] Filtering dengan gf redirect..."
cat "$output_dir/output.txt" | gf redirect | sed 's/=.*/=/' | sort -u > "$output_dir/open_redirect_output.txt"

# pemeriksaan apakah output redirect kosong atau berisi url
if [[ ! -s "$output_dir/open_redirect_output.txt" ]]; then
    echo "⚠ Tidak ditemukan endpoint redirect pada hasil."
...
echo "[✓] Redirect suspected URL di    : $output_dir/open_redirect_output.txt"

# Step 8: Jalankan scanner otomatis
echo
echo "[→] Menjalankan scanner otomatis..."
script_dir="$(dirname "$(realpath "$0")")"
"$script_dir/run_or_auto1.sh" "$output_dir/open_redirect_output.txt" "$session_id"
