...
echo "$(date +%s)" > "/tmp/.xss_timer_${session_id}.tmp"

timestamp="$(date +"%Y%m%d-%H%M%S")_$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 4)"
basedir="$HOME/Documents/projek-or/xss"
...
# Step 7: Cek apakah hasil xss kosong
if [[ ! -s "$output_dir/xss_output.txt" ]]; then
    echo "⚠ Tidak ditemukan endpoint XSS dari hasil crawling."
...
echo "[✓] XSS suspected URL di         : $output_dir/xss_output.txt"

# Step 9: Jalankan scanner otomatis (misal: run_xss_auto1.sh)
echo
echo "[→] Menjalankan scanner otomatis..."
script_dir="$(dirname "$(realpath "$0")")"
"$script_dir/run_xss_auto1.sh" "$output_dir/xss_output.txt" "$session_id"
