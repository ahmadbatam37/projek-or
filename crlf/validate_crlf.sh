...
output_dir="validate_crlf_${timestamp}"
mkdir -p "$output_dir"

validated_output="$output_dir/valid_crlf_verified_${timestamp}.txt"
...
echo "[i] Validasi CRLF Injection menggunakan curl dan grep"
...
while IFS= read -r url; do
  ...
  # Ambil response header
  response=$(curl -skL -i -A "Mozilla/5.0" "$url" --max-time 10 | tr -d '\000')
  header_only=$(echo "$response" | awk '/^\s*$/{exit} {print}')

  # Validasi header rentan
  if echo "$header_only" | grep -i -qE '^set-cookie:.*coffin=hi' \
    || echo "$header_only" | grep -i -qE '^set-cookie:.*loxs=injected' \
    || echo "$header_only" | grep -i -qE '^location:\s*(loxs\.pages\.dev|www\.evil\.com)' \
    || echo "$header_only" | grep -i -qE '^x-xss-protection:\s*0' \
    || echo "$header_only" | grep -i -qE '^coffin-x:\s*coffin-x' \
    || echo "$header_only" | grep -i -qE '^loxs-x:\s*loxs-x'; then

    echo "[✓] VALID CRLF: Ditemukan header injection" | tee -a "$summary_file"
    echo "$url" >> "$validated_output"
    ((valid_count++))
  else
    echo "[x] TIDAK valid: Tidak ditemukan header injection" | tee -a "$summary_file"
  fi
...
echo
clear
echo "──────────── RINGKASAN VALIDASI CRLF ────────────"
echo "Total URL diuji        : $total_checked"
echo "Terbukti rentan CRLF   : $valid_count"
echo
cat "$summary_file"
...
if [[ -s "$validated_output" ]]; then
  ...
  echo
  echo "────────────────────────────────────"
  echo "🔍 Menampilkan rekomendasi mitigasi untuk CRLF Injection:"
  source /home/ahmad/Documents/projek-or/common_function.sh
  get_recommendation "CRLF Injection"
else
  echo "⚠ Tidak ada URL valid ditemukan oleh curl."
fi

# Timer
timer_file="/tmp/.crlf_timer_${session_id}.tmp"
if [[ -f "$timer_file" ]]; then
  ...
  echo
  echo "⏱ Total waktu uji penetrasi CRLF: ${minutes} menit ${seconds} detik"
  rm -f "$timer_file"
fi