...
echo "$(date +%s)" > "/tmp/.crlf_timer_${session_id}.tmp"

timestamp="$(date +"%Y%m%d-%H%M%S")_$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 4)"
basedir="$HOME/Documents/projek-or/crlf"
...
cat "$katana_file" "$paramspider_file" | sort -u > "$output_dir/output_raw.txt"
grep -Ev '\.(css|js|png|jpg|jpeg|svg|gif|woff2?|ttf|eot|ico|mp4|webm|avi|pdf|zip|tar)(\?|$)' "$output_dir/output_raw.txt" > "$output_dir/output.txt"

# Step 6: Filtering endpoint CRLF
cat "$output_dir/output.txt" \
  | gf crlf \
  | sed 's/=.*/=/' \
  | sort -u > "$output_dir/crlf_output_with_param.txt"
echo "[✓] Disimpan ke: $output_dir/crlf_output_with_param.txt"

# 2. URL tanpa parameter (boleh ada path)
cat "$output_dir/output.txt" \
  | grep -v "=" \
  | sort -u > "$output_dir/crlf_output_without_param.txt"
echo "[✓] Disimpan ke: $output_dir/crlf_output_without_param.txt"

# 3. URL root domain saja (tidak ada path dan parameter)
cat "$output_dir/output.txt" | sed 's|/$||' \
  |  grep -v "www\.www\." \
  | awk -F/ '{gsub(/^www\./,"",$3); print $1 "//" $3 "\n" $1 "//www." $3}'\
  | sort -u > "$output_dir/crlf_output_root_only.txt"
#Gabungan
cat "$output_dir/crlf_output_with_param.txt" \
    "$output_dir/crlf_output_without_param.txt" \
    "$output_dir/crlf_output_root_only.txt" \
    | sort -u > "$output_dir/crlf_output.txt"

echo "[✓] Gabungan akhir disimpan ke: $output_dir/crlf_output.txt"

# Step 7: Cek apakah hasil crlf kosong
if [[ ! -s "$output_dir/crlf_output.txt" ]]; then
    echo "⚠ Tidak ditemukan endpoint CRLF dari hasil crawling."
...
echo "[✓] CRLF suspected URL di         : $output_dir/crlf_output.txt"

# Step 9: Jalankan scanner otomatis (misal: run_crlf_auto1.sh)
echo
echo "[→] Menjalankan scanner otomatis..."
script_dir="$(dirname "$(realpath "$0")")"
"$script_dir/run_crlf_auto1.sh" "$output_dir/crlf_output.txt" "$session_id"
