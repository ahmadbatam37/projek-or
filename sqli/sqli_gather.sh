...
# Buat session ID unik dan catat waktu mulai
...
echo "$(date +%s)" > "/tmp/.sqli_timer_${session_id}.tmp"

timestamp="$(date +"%Y%m%d-%H%M%S")_$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 4)"
basedir="$HOME/Documents/projek-or/sqli"
...
if [[ -n "$crawling_dir" && -d "$crawling_dir" ]]; then
    ...
    # Step 1: Siapkan input_url.txt untuk katana
...
# Step 6: Filter kemungkinan SQLi
echo "[•] Filtering dengan gf sqli..."
#versi 1 parameter value kosong
cat "$output_dir/output.txt" | gf sqli | sed 's/=.*/=/' | sort -u > "$output_dir/sqli_output.txt"
# Versi 2: Tambahkan dummy value (misal =1) → untuk sqlmap
cat "$output_dir/sqli_output.txt" | sed 's/=\([&#"]\|$\)/=1\1/g' > "$output_dir/sqli_with_dummy.txt"

# Cek apakah ada hasil
if [[ ! -s "$output_dir/sqli_output.txt" ]]; then
    echo "⚠ Tidak ditemukan parameter mencurigakan untuk SQLi."
    echo "ℹ Tidak perlu menjalankan scanner SQLi."
    exit 0
fi
# Validasi apakah file dummy berhasil dibuat
if [[ ! -s "$output_dir/sqli_with_dummy.txt" ]]; then
    echo "❌ Gagal membuat file dummy untuk sqlmap: $output_dir/sqli_with_dummy.txt"
    exit 1
else
    echo "[✓] Dummy URL untuk sqlmap disimpan di : $output_dir/sqli_with_dummy.txt"
fi
# Step 7: Tampilkan hasil
echo "────────────────────────────────────"
echo "[✓] Semua URL disimpan di       : $output_dir/output.txt"
echo "[✓] SQLi suspected URL di       : $output_dir/sqli_output.txt"

# Step 8: Jalankan scanner otomatis
echo
echo "[→] Menjalankan scanner otomatis SQLi..."
script_dir="$(dirname "$(realpath "$0")")"
"$script_dir/run_sqli_auto1.sh" "$output_dir/sqli_with_dummy.txt" "$output_dir/sqli_output.txt" "$session_id"
